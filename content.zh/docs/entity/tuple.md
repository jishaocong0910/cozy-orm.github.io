...
预定义实体是CozyORM提供的一些类型，可作为实体使用，用于方便查询少量字段，或非表字段（例如聚合函数）等业务，有如下类型。预定义实体只能用于基础执行方法[[Query]](../../execute/base/#query)
...
泛型的数量代表可映射字段的最大数量，且须满足[[字段类型]](../field_type)要求，为了方便，允许指针写成值类型。
