...
| Logger              | `orm.Logger`              | 日志器，用于打印SQL等日志，详见[[日志]](../../log/logger)。                                                                                                                                                       |
...
| QuotedIdentifier    | `orm.QuotedIdentifier`    | 引用标识符，通过枚举`orm.QuotedIdentifier_`选择（见[[引用标识符]](#引用标识符)）。可通过指定`DBType`参数快速配置。                                                                                                |
...
Go原生的`sql.Result.LastInsertId()`不能支持所有数据库，且支持的数据库之间意义也有所不同。`orm.GetGeneratedKeyMode_`收集了各种获取生成Key的方式，有以下选项。
...
基础执行方法`orm.DB.Mutation`只支持`FirstInsertId`和`LastInsertId`，高级执行方法`orm.DB.Insert`支持所有。
