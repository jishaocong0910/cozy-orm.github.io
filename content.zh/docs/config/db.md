...
Go原生的`sql.Result.LastInsertId()`不能支持所有数据库，且支持的数据库之间逻辑意义也有所不同。`orm.GetGeneratedKeyMode_`总结了各种获取生成Key的方式，有以下选项。
...
基础执行方法`orm.DB.Mutation`只支持`FirstInsertId`和`LastInsertId`，高级执行方法`orm.DB.Insert`支持所有，详见[[Mutation]]()[[Insert]]()。
